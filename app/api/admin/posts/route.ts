...
export async function POST(request: NextRequest) {
  ...
  try { const body = await request.json(); const title = text(body.title, 140); const constellationSlug = text(body.constellation, 100); const selected = constellations.find(item => item.slug === constellationSlug); const summary = text(body.summary, 300); const date = text(body.date, 40); const publishedAt = text(body.publishedAt, 10); const readTime = text(body.readTime, 20); const requestedImage = text(body.image, 1000); const image = requestedImage.startsWith("/") || requestedImage.startsWith("https://") ? requestedImage : "/stardust-hero.png"; const content = Array.isArray(body.content) ? body.content.map((item: unknown) => text(item, 3000)).filter(Boolean).slice(0, 30) : [];
    ...
    const post: Post = { slug, constellation: selected.slug, title, image, date, publishedAt: /^\d{4}-\d{2}-\d{2}$/.test(publishedAt) ? publishedAt : undefined, category: selected.title, summary, readTime, content }; await storePost(post); return NextResponse.json(post, { status: 201 });
