...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; const posts = await getPosts() as Post[]; const post = posts.find(p => p.slug === slug); if (!post) notFound(); const postComments = typedComments.filter(c => c.postSlug === post.slug); return <main className="article-page"><div className="article-hero"><ArticleImage src={post.image} alt={`Imagen de ${post.title}`} priority className="article-bg" /><div className="article-overlay" /><Stars /><div className="article-heading"><Link href={`/constelaciones/${post.constellation}`}><ArrowLeft size={17} /> Volver a la constelación</Link><span className="kicker">{post.category}</span><h1>{post.title}</h1><p>{post.summary}</p><small>{post.date} · <Clock size={14} /> {post.readTime} de lectura</small></div></div><article className="article-content"><div className="article-icon"><Sparkles /></div>{post.content.map((paragraph, i) => <p key={i}>{paragraph}</p>)}</article><CommentSection postSlug={post.slug} initialComments={postComments} /></main>; }
