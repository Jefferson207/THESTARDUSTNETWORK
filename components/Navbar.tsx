...
export function Navbar() {
  ...
  return <header className={`nav ${scrolled ? "nav-scrolled" : ""}`}>
    ...
    {open && <nav className="mobile-menu"><small>CONSTELACIONES</small>{constellations.map(item => <Link key={item.slug} onClick={() => setOpen(false)} href={`/#${item.slug}`}>{item.title}</Link>)}<Link onClick={() => setOpen(false)} href="/#unirme">Unirme a la comunidad</Link></nav>}
  </header>;
}
